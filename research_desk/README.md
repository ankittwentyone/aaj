# Research Desk — fixed-pipeline LangGraph agent + Groq (Hermes retired)
...
model (`ChatGroq`, `LLM_API_KEY` groq, cloud inference only).
...
- Without `LLM_API_KEY` (groq), LLM nodes stub (follow-up → None, synthesis →
